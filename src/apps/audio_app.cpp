/**
 * @file audio_app.cpp
 * @brief Giao diện ứng dụng Voice AI & Audio Lab trên màn hình 3.5" IPS 480x320
 * Tích hợp điều khiển Loa ngoài FM8002E, Microphone MEMS, Soundboard, Voice Recorder PSRAM và XiaoZhi AI
 */
...
static void volume_slider_cb(lv_event_t *e)
{
    ...
    if (lbl_vol_val)
    {
        lv_label_set_text_fmt(lbl_vol_val, "🔊 Âm Lượng Loa: %d%%", val);
    }
}
...
static void record_btn_cb(lv_event_t *e)
{
    if (audio_is_recording())
    {
        ...
        lv_label_set_text(lbl_record_btn, LV_SYMBOL_PLAY " Thu Âm");
        lv_obj_set_style_bg_color(btn_record, lv_color_hex(0xFF3B30), 0);
        if (lbl_recorder_status)
        {
            ...
            lv_obj_set_style_text_color(lbl_recorder_status, lv_color_hex(0x00E676), 0);
        }
    }
    else
    {
        ...
        {
            lv_label_set_text(lbl_record_btn, LV_SYMBOL_STOP " Dừng Thu");
            lv_obj_set_style_bg_color(btn_record, lv_color_hex(0xE53E3E), 0);
            if (lbl_recorder_status)
            {
                lv_label_set_text(lbl_recorder_status, "🔴 Đang thu âm từ Mic...");
                lv_obj_set_style_text_color(lbl_recorder_status, lv_color_hex(0xFF3B30), 0);
            }
        }
        else
        {
            if (lbl_recorder_status)
            {
                lv_label_set_text(lbl_recorder_status, "⚠️ Lỗi bộ nhớ PSRAM!");
                lv_obj_set_style_text_color(lbl_recorder_status, lv_color_hex(0xFFB300), 0);
            }
        }
    }
}
...
static void play_btn_cb(lv_event_t *e)
{
    ...
    {
        audio_stop_playback();
        lv_label_set_text(lbl_play_btn, LV_SYMBOL_AUDIO " Phát Loa");
        lv_obj_set_style_bg_color(btn_play, lv_color_hex(0x00E676), 0);
        if (lbl_recorder_status)
        {
            lv_label_set_text(lbl_recorder_status, "⏹ Đã dừng phát lại");
            lv_obj_set_style_text_color(lbl_recorder_status, lv_color_hex(0xA0AEC0), 0);
        }
    }
    else
    {
        ...
        {
            lv_label_set_text(lbl_play_btn, LV_SYMBOL_STOP " Dừng Phát");
            lv_obj_set_style_bg_color(btn_play, lv_color_hex(0x38A169), 0);
            if (lbl_recorder_status)
            {
                lv_label_set_text(lbl_recorder_status, "🟢 Đang phát lại qua Loa...");
                lv_obj_set_style_text_color(lbl_recorder_status, lv_color_hex(0x00E676), 0);
            }
        }
        else
        {
            if (lbl_recorder_status)
            {
                lv_label_set_text(lbl_recorder_status, "⚠️ Chưa có đoạn ghi âm!");
                lv_obj_set_style_text_color(lbl_recorder_status, lv_color_hex(0xFFB300), 0);
            }
        }
    }
...
/* =========================================================================
 * KHỞI TẠO VÀ MỞ ỨNG DỤNG AUDIO LAB (480x266)
 * ========================================================================= */
void audio_app_open(lv_obj_t *parent)
{
    ...
    // =========================================================================
    // CỘT TRÁI (W: 232, H: 254): MICROPHONE LIVE OSCILLOSCOPE & VU METER
    // =========================================================================
    lv_obj_t *card_mic = lv_obj_create(parent);
    lv_obj_set_size(card_mic, 232, 254);
    lv_obj_set_pos(card_mic, 2, 2);
    lv_obj_set_style_radius(card_mic, 12, 0);
    lv_obj_set_style_bg_color(card_mic, lv_color_hex(0x161B26), 0);
    lv_obj_set_style_border_color(card_mic, lv_color_hex(0x00F2FE), 0);
    lv_obj_set_style_border_width(card_mic, 1, 0);
    lv_obj_set_style_pad_all(card_mic, 8, 0);
    lv_obj_clear_flag(card_mic, LV_OBJ_FLAG_SCROLLABLE);

    // Tiêu đề Card Mic
    lv_obj_t *t_mic = lv_label_create(card_mic);
    lv_label_set_text(t_mic, "🎙️ Microphone MEMS (Live)");
    lv_obj_set_style_text_color(t_mic, lv_color_hex(0x00F2FE), 0);
    lv_obj_set_style_text_font(t_mic, &lv_font_montserrat_12, 0);
    lv_obj_align(t_mic, LV_ALIGN_TOP_LEFT, 0, 0);

    // VU Meter Bar (Thanh cường độ âm thanh)
    bar_vu_meter = lv_bar_create(card_mic);
    lv_obj_set_size(bar_vu_meter, 214, 12);
    lv_obj_align(bar_vu_meter, LV_ALIGN_TOP_MID, 0, 20);
    ...
    lv_obj_set_style_bg_color(bar_vu_meter, lv_color_hex(0x00E676), LV_PART_INDICATOR);
    lv_obj_set_style_radius(bar_vu_meter, 4, 0);

    // Nhãn hiển thị dB & Mức thu
    lbl_vu_val = lv_label_create(card_mic);
    lv_label_set_text(lbl_vu_val, "Mức thu: 0% | -60.0 dB (Im lặng)");
    lv_obj_set_style_text_color(lbl_vu_val, lv_color_hex(0xA0AEC0), 0);
    lv_obj_set_style_text_font(lbl_vu_val, &lv_font_montserrat_10, 0);
    lv_obj_align(lbl_vu_val, LV_ALIGN_TOP_LEFT, 0, 36);

    // Biểu đồ sóng âm thanh thời gian thực (Real-time Oscilloscope)
    chart_waveform = lv_chart_create(card_mic);
    lv_obj_set_size(chart_waveform, 214, 150);
    lv_obj_align(chart_waveform, LV_ALIGN_BOTTOM_MID, 0, -22);
    lv_chart_set_type(chart_waveform, LV_CHART_TYPE_LINE);
    lv_chart_set_point_count(chart_waveform, 48);
    lv_chart_set_range(chart_waveform, LV_CHART_AXIS_PRIMARY_Y, -3000, 3000);
    lv_chart_set_div_line_count(chart_waveform, 5, 5);
    ...
    ser_waveform = lv_chart_add_series(chart_waveform, lv_color_hex(0x00F2FE), LV_CHART_AXIS_PRIMARY_Y);
    for (int i = 0; i < 48; i++)
    {
        lv_chart_set_next_value(chart_waveform, ser_waveform, 0);
    }

    // Ghi chú chân phần cứng
    lv_obj_t *lbl_hw = lv_label_create(card_mic);
    lv_label_set_text(lbl_hw, "I2S DIN: GPIO 16 • WS: 21 • BCK: 18");
    lv_obj_set_style_text_color(lbl_hw, lv_color_hex(0x718096), 0);
    lv_obj_set_style_text_font(lbl_hw, &lv_font_montserrat_10, 0);
    lv_obj_align(lbl_hw, LV_ALIGN_BOTTOM_LEFT, 0, 0);

    // =========================================================================
    // CỘT PHẢI (W: 232, H: 254): SPEAKER CONTROL, SOUNDBOARD & PSRAM VOICE MEMO
    // =========================================================================
    lv_obj_t *card_spk = lv_obj_create(parent);
    lv_obj_set_size(card_spk, 232, 254);
    lv_obj_set_pos(card_spk, 238, 2);
    lv_obj_set_style_radius(card_spk, 12, 0);
    lv_obj_set_style_bg_color(card_spk, lv_color_hex(0x161B26), 0);
    lv_obj_set_style_border_color(card_spk, lv_color_hex(0xFFB300), 0);
    lv_obj_set_style_border_width(card_spk, 1, 0);
    lv_obj_set_style_pad_all(card_spk, 8, 0);
    lv_obj_clear_flag(card_spk, LV_OBJ_FLAG_SCROLLABLE);

    // Slider Volume
    lbl_vol_val = lv_label_create(card_spk);
    lv_label_set_text_fmt(lbl_vol_val, "🔊 Âm Lượng Loa: %d%%", audio_get_volume());
    ...
    lv_obj_set_size(slider_vol, 214, 10);
    lv_obj_align(slider_vol, LV_ALIGN_TOP_MID, 0, 20);
    lv_slider_set_range(slider_vol, 0, 100);
    lv_slider_set_value(slider_vol, audio_get_volume(), LV_ANIM_OFF);
    lv_obj_set_style_bg_color(slider_vol, lv_color_hex(0xFFB300), LV_PART_INDICATOR);
    lv_obj_set_style_bg_color(slider_vol, lv_color_hex(0xFFD54F), LV_PART_KNOB);
    lv_obj_add_event_cb(slider_vol, volume_slider_cb, LV_EVENT_VALUE_CHANGED, nullptr);

    // Tiêu đề Soundboard
    lv_obj_t *t_sb = lv_label_create(card_spk);
    lv_label_set_text(t_sb, "🔔 Thử Loa Ngoài (Soundboard):");
    lv_obj_set_style_text_color(t_sb, lv_color_hex(0xFFFFFF), 0);
    lv_obj_set_style_text_font(t_sb, &lv_font_montserrat_10, 0);
    lv_obj_align(t_sb, LV_ALIGN_TOP_LEFT, 0, 36);

    // 4 Nút Soundboard (Chime, Beep, Melody, XiaoZhi)
    auto create_snd_btn = [&](const char *txt, int x, int y, uint32_t hex, SoundEffect fx) {
        lv_obj_t *b = lv_btn_create(card_spk);
        lv_obj_set_size(b, 102, 28);
    ...
    create_snd_btn(LV_SYMBOL_BELL " Chime",  2,   52, 0x1E3A8A, FX_CHIME);
    create_snd_btn(LV_SYMBOL_CHARGE " Beep", 110, 52, 0x7C2D12, FX_BEEP);
    create_snd_btn(LV_SYMBOL_AUDIO " Melody",2,   84, 0x14532D, FX_MELODY);
    create_snd_btn("🤖 XiaoZhi",              110, 84, 0x581C87, FX_XIAOZHI_WAKE);

    // VÙNG GHI ÂM PSRAM (VOICE MEMO / DICTAPHONE)
    lv_obj_t *memo_box = lv_obj_create(card_spk);
    lv_obj_set_size(memo_box, 214, 112);
    lv_obj_align(memo_box, LV_ALIGN_BOTTOM_MID, 0, 0);
    lv_obj_set_style_radius(memo_box, 8, 0);
    lv_obj_set_style_bg_color(memo_box, lv_color_hex(0x111622), 0);
    lv_obj_set_style_border_color(memo_box, lv_color_hex(0x2D3748), 0);
    ...
    lv_label_set_text(t_memo, "🎙️ Ghi & Phát Lại (8MB PSRAM):");
    lv_obj_set_style_text_color(t_memo, lv_color_hex(0x00F2FE), 0);
    lv_obj_set_style_text_font(t_memo, &lv_font_montserrat_10, 0);
    lv_obj_align(t_memo, LV_ALIGN_TOP_LEFT, 0, 0);

    // 2 Nút Ghi Âm và Phát Lại
    btn_record = lv_btn_create(memo_box);
    lv_obj_set_size(btn_record, 96, 30);
    lv_obj_set_pos(btn_record, 2, 20);
    ...
    lv_obj_set_size(btn_play, 96, 30);
    lv_obj_set_pos(btn_play, 104, 20);
    ...
    // Trạng thái bộ ghi âm
    lbl_recorder_status = lv_label_create(memo_box);
    lv_label_set_text(lbl_recorder_status, "Trạng thái: Sẵn sàng (Tối đa 10s)");
    lv_obj_set_style_text_color(lbl_recorder_status, lv_color_hex(0xA0AEC0), 0);
    lv_obj_set_style_text_font(lbl_recorder_status, &lv_font_montserrat_10, 0);
    lv_obj_align(lbl_recorder_status, LV_ALIGN_BOTTOM_LEFT, 2, -16);

    // Tiến trình ghi / phát
    bar_record_progress = lv_bar_create(memo_box);
    lv_obj_set_size(bar_record_progress, 200, 6);
    lv_obj_align(bar_record_progress, LV_ALIGN_BOTTOM_MID, 0, -2);
    ...
    lv_obj_set_style_bg_color(bar_record_progress, lv_color_hex(0x00F2FE), LV_PART_INDICATOR);
}
...
void audio_app_update(void)
{
    ...
    if (bar_vu_meter)
    {
        ...
        // Đổi màu VU Bar theo cường độ: Xanh lá -> Vàng -> Đỏ
        if (mic_level > 80)
            lv_obj_set_style_bg_color(bar_vu_meter, lv_color_hex(0xFF3B30), LV_PART_INDICATOR);
        else if (mic_level > 40)
            lv_obj_set_style_bg_color(bar_vu_meter, lv_color_hex(0xFFB300), LV_PART_INDICATOR);
        else
            lv_obj_set_style_bg_color(bar_vu_meter, lv_color_hex(0x00E676), LV_PART_INDICATOR);
    }
    ...
    if (lbl_vu_val)
    {
        ...
        if (mic_level > 60) state_txt = "Âm lượng cao!";
        else if (mic_level > 20) state_txt = "Đang nói...";
        lv_label_set_text_fmt(lbl_vu_val, "Mức thu: %d%% | %.1f dB (%s)", mic_level, mic_db, state_txt);
    }

    // 2. Cập nhật biểu đồ sóng Oscilloscope
    int16_t wave_samples[48];
    audio_get_waveform_samples(wave_samples, 48);
    for (int i = 0; i < 48; i++)
    {
        lv_chart_set_next_value(chart_waveform, ser_waveform, wave_samples[i]);
    }

    // 3. Cập nhật trạng thái Ghi Âm / Phát Lại
