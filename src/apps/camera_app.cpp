 ...
 * Ưu tiên vùng ảnh Canvas lớn, toolbar nhỏ phía dưới, hiển thị đúng năng lực (Capabilities)
 */
...
void camera_app_open(lv_obj_t *parent)
{
    ...
    if (cam_canvas_buf)
    {
        // Khởi tạo màu xám tối cho canvas
        for (int i = 0; i < canvas_w * canvas_h; i++)
        {
            cam_canvas_buf[i] = lv_color_hex(0x121622);
        }
    }
    ...
    // Hàng nút điều khiển nhỏ: Chụp lại / Cấu hình / Ngắt
    lv_obj_t *btn_row = lv_obj_create(toolbar_box);
    lv_obj_set_size(btn_row, SCREEN_WIDTH - 16, 32);
    ...
    lv_obj_set_size(btn_snap, 72, 30);
    ...
    lv_obj_set_size(btn_disconnect, 68, 30);
    ...
    lv_obj_set_size(btn_m_close, 26, 22);
    lv_obj_align(btn_m_close, LV_ALIGN_TOP_RIGHT, -2, -2);
    ...
    auto make_input = [&](const char *label_text, const char *default_val, lv_coord_t y_pos, bool is_pwd = false) -> lv_obj_t* {
        ...
        lv_obj_set_size(ta, SCREEN_WIDTH - 28, 28);
    ...
    ta_name = make_input("Tên Camera:", "IP Cam Phòng Khách", 26);
    ta_ip = make_input("Địa chỉ IP:", "192.168.1.50", 72);
    ta_http_port = make_input("HTTP Snapshot Port:", "80", 118);
    ta_rtsp_port = make_input("RTSP Port:", "554", 164);
    ta_onvif_port = make_input("ONVIF Port:", "8000", 210);
    ta_user = make_input("Tài khoản (Username):", "admin", 256);
    ta_pass = make_input("Mật khẩu (Password):", "admin123", 302, true);
    ...
    lv_obj_set_size(dd_vendor, SCREEN_WIDTH - 28, 28);
    ...
    lv_obj_set_size(dd_proto, SCREEN_WIDTH - 28, 28);
    ...
    lv_obj_set_size(btn_save_connect, SCREEN_WIDTH - 28, 32);
    lv_obj_set_pos(btn_save_connect, 6, 448);
...
void camera_app_update(void)
{
    ...
    if (frame && frame->buf && frame->len > 0)
    {
        ...
        uint32_t latency = now - last_frame_time_ms;
        ...
        if (lbl_metrics)
        {
            lv_label_set_text_fmt(lbl_metrics, "FPS: %.1f • %dx%d • %uKB • %ums",
                real_fps,
                frame->width > 0 ? frame->width : canvas_w,
                frame->height > 0 ? frame->height : canvas_h,
