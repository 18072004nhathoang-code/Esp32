...
static void power_manager_apply_brightness_serialized(void)
{
    if (s_brightness_mutex)
    {
        xSemaphoreTake(s_brightness_mutex, portMAX_DELAY);
    }
    ...
    while (true)
    {
        ...
        needs_update = s_brightness_coordinator.get_next_hardware_target(&target_b, &target_rev);
        ...
        lvgl_port_set_brightness(target_b);

        portENTER_CRITICAL(&power_mux);
        s_brightness_coordinator.commit_hardware_applied(target_rev, target_b);
        portEXIT_CRITICAL(&power_mux);
    }

    if (s_brightness_mutex)
    {
        xSemaphoreGive(s_brightness_mutex);
    }
}
...
void power_manager_init(void)
{
    if (!s_brightness_mutex)
    {
        s_brightness_mutex = xSemaphoreCreateMutex();
    }
    ...
    s_brightness_coordinator.post_intent(user_active_brightness);
    portEXIT_CRITICAL(&power_mux);

    // Thiết lập độ sáng ban đầu 100%
    power_manager_apply_brightness_serialized();
...
void power_manager_wake(void)
{
    ...
    // Khôi phục độ sáng qua serializer
    power_manager_apply_brightness_serialized();
}
...
void power_manager_sleep(void)
{
    ...
    ++activity_revision;
    s_brightness_coordinator.post_intent(POWER_BRIGHTNESS_SLEEP_DEFAULT);
    portEXIT_CRITICAL(&power_mux);
...
void power_manager_update(void)
{
    ...
    if (target_state != state_snapshot)
    {
        ...
        if (power_manager_can_apply_transition(activity_revision, rev_snapshot, current_power_state, state_snapshot))
        {
            ...
            else if (target_state == POWER_STATE_DIMMED)
            {
                s_brightness_coordinator.post_intent(POWER_BRIGHTNESS_DIM_DEFAULT);
            }
            ++activity_revision;
            applied = true;
        }
