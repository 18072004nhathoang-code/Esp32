...
static int ws_read_header(esp_transport_handle_t t, char *buffer, int len, int timeout_ms)
{
    ...
    const uint8_t raw_header_byte0 = (uint8_t)(*data_ptr);
    const uint8_t pure_opcode = (raw_header_byte0 & 0x0F);
    const bool is_fin = (raw_header_byte0 & 0x80) != 0;
    // For control frames (CLOSE 0x08, PING 0x09, PONG 0x0A): keep pure opcode (bits 0..3) so internal
    // transport handlers and esp_websocket_client comparisons (== 0x08, 0x09, 0x0A) never break.
    // For data frames (CONT 0x00, TEXT 0x01, BINARY 0x02): preserve true FIN bit (0x80) so XiaozhiTransport
    // receives accurate FIN metadata without bitwise guessing.
    ws->frame_state.opcode = (pure_opcode & WS_OPCODE_CONTROL_FRAME)
        ? pure_opcode
        : (pure_opcode | (is_fin ? 0x80 : 0x00));
    data_ptr ++;
