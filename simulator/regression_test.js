...
const wifiScanCoordinator = source('include/wifi_scan_coordinator.h');
const wifiScanAdapter = source('src/os/wifi_scan_adapter.cpp');
const wifiFrameworkPatch = source('scripts/framework_wifi_patch.py');
const xiaozhiActivation = source('src/ai/xiaozhi_activation.cpp');
const timeService = source('src/os/time_service.cpp');
const secretsExample = source('include/secrets.example.h');
...
assert.match(wifiScanCoordinator, /WAITING_FOR_RADIO/);
assert.match(wifiService, /WifiScanDriverAdapter/);
assert.match(wifiService, /WIFI_SCAN_TOTAL_BUDGET_MS/);
assert.match(wifiScanAdapter, /wifi_scan_zero_config/);
assert.match(wifiScanAdapter, /esp_wifi_scan_start\(&config, false\)/);
assert.match(wifiScanAdapter, /esp_wifi_scan_stop/);
assert.match(wifiFrameworkPatch, /EXPECTED_HASHES/);
assert.match(wifiFrameworkPatch, /home_chan_dwell_time/);
assert.match(wifiFrameworkPatch, /Initial reconnect delegated to application worker/);
assert.match(wifiService, /WiFi\.setAutoReconnect\(false\)/);
assert.match(xiaozhiActivation, /createNestedObject\("chip_info"\)/);
assert.match(xiaozhiActivation, /createNestedArray\("partition_table"\)/);
assert.match(xiaozhiActivation, /esp_ota_get_running_partition/);
assert.match(xiaozhiActivation, /createNestedObject\("board"\)/);
assert.doesNotMatch(secretsExample, /Your_WiFi/);
assert.match(timeService, /configTzTime/);
...
assert.doesNotMatch(platformio, /ESP32-audioI2S\.git/);
assert.match(source('lib/ESP32-audioI2S-patched/UPSTREAM.md'), /928c420d49fce2a09fa91f490b9fcabed6447c67/);
assert.match(source('lib/ESP32-audioI2S-patched/src/Audio.cpp'), /shutdown\(portMAX_DELAY\)/);
assert.match(source('lib/ESP32-audioI2S-patched/src/Audio.cpp'), /m_audioTaskExitAck/);
