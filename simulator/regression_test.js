...
assert.match(touch, /map_calibrated_touch\(calibration, raw_x, raw_y/);
assert.match(touch, /prefs\.getBytes\("data", &loaded, sizeof\(loaded\)\)/);
assert.match(touch, /Safe profile-based fallback; a valid saved affine calibration supersedes it/);
assert.doesNotMatch(touch, /shared_i2c_touch_(solve|commit_calibration|reset_calibration|read_debug|set_ui_suppressed)/);
assert.strictEqual(fs.existsSync(path.join(root, 'src/ui/touch_test.cpp')), false);
assert.strictEqual(fs.existsSync(path.join(root, 'src/ui/touch_test.h')), false);
assert.doesNotMatch(ui, /Touch Test|Touch Calibration|APP_TOUCH_TEST|ui_touch_test_/);
assert.doesNotMatch(main, /ui_touch_test_|digitalRead\(BOARD_BOOT_PIN\)|WiFi deferred until touch calibration/);
