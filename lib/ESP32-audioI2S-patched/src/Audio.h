...
public:
    ...
    ~Audio();
    bool isInitialized() const { return m_f_initialized; }
    bool shutdown(uint32_t timeout_ms = 1000);
...
private:
  ...
  bool            startAudioTask(); // starts a task for decode and play
  bool            stopAudioTask(uint32_t timeout_ms); // request stop and await task acknowledgement
...
#pragma GCC diagnostic pop
    ...
    volatile bool   m_f_audioTaskIsRunning = false;
    bool            m_f_initialized = false;
    bool            m_f_i2sInstalled = false;
