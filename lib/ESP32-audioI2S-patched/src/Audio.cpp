...
Audio::Audio(bool internalDAC /* = false */, uint8_t channelEnabled /* = I2S_SLOT_MODE_STEREO */, uint8_t i2sPort) {
    ...
    m_audioTaskExitAck = xSemaphoreCreateBinary();
...
#define __malloc_heap_psram(size) \
    ...
    if(!mutex_playAudioData || !m_audioTaskExitAck || !m_chbuf || !m_lastHost || !m_outBuff || !m_ibuff) {
        log_e("Audio initialization failed: allocation");
        return;
    }
...
#define AUDIO_INFO(...)                     \
    ...
    if(i2s_new_channel(&m_i2s_chan_cfg, &m_i2s_tx_handle, NULL) != ESP_OK) {
        log_e("Audio initialization failed: i2s_new_channel");
        return;
    }
    m_f_i2sInstalled = true;
    ...
    if(i2s_channel_init_std_mode(m_i2s_tx_handle, &m_i2s_std_cfg) != ESP_OK || I2Sstart(0) != ESP_OK) {
        log_e("Audio initialization failed: I2S channel configuration");
        return;
    }
    m_sampleRate = 44100;
    ...
    if (internalDAC)  {
        ...
        if(i2s_driver_install((i2s_port_t)m_i2s_num, &m_i2s_config, 0, NULL) != ESP_OK) {
            log_e("Audio initialization failed: I2S driver");
            return;
        }
        m_f_i2sInstalled = true;
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Audio::~Audio() {
    // Normal callers use shutdown(timeout) and retain this object on timeout.
    // This unbounded fallback prevents direct delete from racing PeriodicTask.
    (void)shutdown(portMAX_DELAY);
    // I2Sstop(m_i2s_num);
    // InBuff.~AudioBuffer(); #215 the AudioBuffer is automatically destroyed by the destructor
    if(m_ibuff && m_chbuf && m_lastHost && m_outBuff) setDefaults();
    if(m_playlistBuff) {
        free(m_playlistBuff);
        m_playlistBuff = NULL;
    }
#if ESP_IDF_VERSION_MAJOR == 5
    if(m_f_i2sInstalled && m_i2s_tx_handle) i2s_del_channel(m_i2s_tx_handle);
#else
...
bool Audio::shutdown(uint32_t timeout_ms) {
    ...
    if(m_outBuff) stopSong();
    xSemaphoreGive(mutex_playAudioData);
    const bool stopped = stopAudioTask(timeout_ms);
    if(stopped) m_f_initialized = false;
    return stopped;
}

uint32_t Audio::getAudioTaskStackHighWaterMark() const {
    TaskHandle_t handle = m_audioTaskHandle;
    return handle ? static_cast<uint32_t>(uxTaskGetStackHighWaterMark(handle)) : 0;
}
